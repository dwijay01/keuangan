...
$user = User::first();
if (!$user) {
    echo "Tidak ada user. Silakan register dulu.\n";
    return;
}
...
// Cek Transaksi (Arus Kas)
$txCount = Transaction::where('user_id', $user->id)->count();
if ($txCount < 5) {
    echo "Menambahkan dummy transaksi (cashflow 3 bulan terakhir)...\n";
    for ($i = 0; $i < 12; $i++) {
...
// Cek Bill (Kalender Bebas Cicilan)
...
// Cek Family Members (Timeline Anak)
